...
import main.java.com.DeliverySystem.other.DataLoader;
import main.java.com.DeliverySystem.people.Recipient;
import main.java.com.DeliverySystem.people.Sender;
    ...
    public void determineShippingPlan() throws UnloadedDataException {
        ...
        try {
            if (!priority && travelRoute.getDistance() < 1000 && shippingPackage.getWeight() <= 150 && shippingPackage.getBox().getArea() <= 65000)
                vehicle = new Truck(DataLoader.getVehicles().get(0), "00-" + vehicleNumber); //Standard Delivery Car
            else if (!priority && travelRoute.getDistance() < 1000 && shippingPackage.getWeight() <= 300 && shippingPackage.getBox().getArea() <= 90000)
                vehicle = new Truck(DataLoader.getVehicles().get(1), "01-" + vehicleNumber); //Heavy Delivery Truck
            else if (priority && travelRoute.getDistance() < 1000 && shippingPackage.getWeight() <= 150 && shippingPackage.getBox().getArea() <= 65000)
                vehicle = new Truck(DataLoader.getVehicles().get(2), "02-" + vehicleNumber); //Priority Delivery Car
            else if (!priority && shippingPackage.getWeight() <= 1000 && shippingPackage.getBox().getArea() <= 500000.0)
                vehicle = new Truck(DataLoader.getVehicles().get(3), "03-" + vehicleNumber); //Delivery Freight Truck

            else if (shippingPackage.getWeight() <= 300 && shippingPackage.getBox().getArea() <= 90000)
                vehicle = new Plane(DataLoader.getVehicles().get(4), "04-" + vehicleNumber); //Standard Delivery Plane
            else vehicle = new Plane(DataLoader.getVehicles().get(5), "05-" + vehicleNumber); //Heavy Cargo Plane
